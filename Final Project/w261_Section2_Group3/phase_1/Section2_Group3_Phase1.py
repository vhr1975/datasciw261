...
# MAGIC | Submit Notebook and PDF                                                                           | Phase 1 | Rathin Bector                                               | 10/30/2022 | TO DO  |
# MAGIC | Create Post of Discussion Page                                                                    | Phase 1 | Rathin Bector                                               | 10/30/2022 | TO DO  |
...
# MAGIC 
# MAGIC The weather dataset comprises 124 fields detailing data such as Daily Average Temperature, Sea Level Pressure, Wind Speed, and Relative Humidity, among others, observed for a variety of places during our sampling period in the first quarter of 2015. 
# MAGIC 
# MAGIC Upon further analysis, we observed that 123 of the 124 fields of the dataset were of string data type and that 109 fields had 80% or more missing values. Based on the above, we performed the following:  
# MAGIC 
# MAGIC 1. Disregarded the 109 fields with 80% or more Null values given that we believe that any attempt to impute any missing data could result in artificially transforming its nature.
# MAGIC 2. Cast the 15 remaining features to the appropriate data type. 
# MAGIC 
# MAGIC Upon examination of the resulting dataset, and contingent on a deeper data exploration and analysis to be performed in Phase II, we concluded that the following features could be used in our model (either as predictors or identifiers for joining with other datasets):
# MAGIC 
# MAGIC STATION: weather station identifier  
# MAGIC DATE: timestamp of the record  
# MAGIC LATITUDE: latitude coordinates   
# MAGIC LONGITUDE: longitude coordinates   
# MAGIC ELEVATION:  elevation relative to mean sea level  
# MAGIC NAME: name of the meteorological-point-observation  
# MAGIC HourlyDewPointTemperature: the temperature the air needs to be cooled to (at constant pressure) in order to achieve a relative humidity (RH) of 100%  
# MAGIC HourlyDryBulbTemperature: ambient air temperature. It is called "Dry Bulb" because the air temperature is indicated by a thermometer not affected by the moisture of the air.  
# MAGIC HourlyRelativeHumidity:  it is the amount of water vapor present in air expressed as a percentage (%RH) of the amount needed to achieve saturation at the same temperature.  
# MAGIC HourlyWindDirection: wind direction in tens of degrees  
# MAGIC HourlyWindSpeed: wind speed expressed in Beaufort scale   
# MAGIC 
# MAGIC ### Weather Stations Dataset
# MAGIC 
# MAGIC The provided weather station dataset contains observations taken by U.S. Air Force personnel at bases in the United States. Stations are assigned a WBAN (Weather Bureau Army Navy) number. Original forms sent from the Air Force to NCDC by agreement and stored in the NCDC archives. The data consists of 12 colums and 5,004,169 rows of weather station data. The data features consist of strings and doubles with no NAN or NULL values. 
# MAGIC 
# MAGIC For the data develpment, we will are planning on joining this dataset using an external mapping table. We will be utilizing the stations latitude and longitude coordinates to find the nearest airport to the weather stations. 
# MAGIC 
# MAGIC We will take advantage of the neighbor_lat, neighbor_lon and distance_toneighbor to explore the idea of creating a distance graph. 
...
# MAGIC 
# MAGIC ## Weather Staton Plot
# MAGIC 
# MAGIC In reviewing the station data, we discovered the latitude, longitude columns for all the weather stations. Using the latitude and longitude columns we generated a table with all the required coordinates. Leveraging the data bricks Map (Marker) visualization tool, we plotted all the station coordinates on a satellite layer. 
# MAGIC 
# MAGIC 
# MAGIC 
# MAGIC <img src="https://drive.google.com/uc?id=12KwK_cdQW0WhIXDRCHol8bxkpMwqxYWj" alt="Google Drive Image" width=90%/>
# MAGIC 
# MAGIC ## Airlines EDA
