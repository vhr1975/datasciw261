...
df_weather = df_weather.withColumn('TimeLag', F.to_timestamp(df_weather.DATE + F.expr('INTERVAL 120 MINUTES')))
#                        .filter(df_weather.STATION.isin(unique_stations)).filter(df_weather.Day.isin(unique_days))\

# COMMAND ----------

df_weather = df_weather.withColumn("Day", split(col("DATE"),"T")[0]).filter((df_weather.NAME.contains(' US')) | (df_weather.NAME.contains(' PR')))\
                       .withColumn("TimeLagRoundedHour", hour((F.ceil(unix_timestamp("TimeLag")/3600)*3600).cast("timestamp"))).cache()
df_weather = df_weather.withColumn("TimeLagFormatted", f'{split(df_weather.TimeLag,"T")[0]}T{split(split(df_weather.TimeLag,"T")[1], ":")[1]}:{df_weather.TimeLagRoundedHour}:00')
...
df_airlines_date_adjust = df_airlines_with_stations.withColumn('CRS_DEP_TIME_PAD', format_string("%04d", col('CRS_DEP_TIME').cast('int')))\
                                                   ...
                                                   .withColumn("DateTimeUTC", hour((f.floor(unix_timestamp("DateTimeUTC")/3600)*3600).cast("timestamp"))).cache()
...
df_weather = df_weather.drop('YEAR')
